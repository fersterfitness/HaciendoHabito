...
export type StudentFormContentProps = {
  ...
  studentId?: string
  onCancel: () => void
  /** id del alumno creado o actualizado */
  onSuccess: (id: string) => void
  /** `modal`: más ancho en columna doble, rejillas más aireadas */
  variant?: 'page' | 'modal'
}
...
export function StudentFormContent({
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (isEditing && studentId) {
      ...
      if (result) onSuccess(studentId)
    } else {
      const result = await createStudent(payload)
      if (result) onSuccess(result.id)
    }
  }
  ...
  const secClass = isModal ? 'space-y-5' : undefined

  /** Layout página (una columna); el modal duplica el markup en dos columnas — mantener alineados. */
  const fields: ReactNode = (
    <>
      <FormSection title="Datos personales" className={secClass}>
        <Input
      ...
      <FormSection title="Estado y plan" className={secClass}>
        <div className={g2}>
          <Select
            label="Nivel"
            required
      ...
      <FormSection title="Entrenamiento y objetivo" className={secClass}>
        <div className={g2}>
          <Select
      ...
      <FormSection title="Salud y hábitos" className={secClass}>
        <Select
      ...
      <FormSection title="Observaciones" className={secClass}>
        <Textarea
  ...
  return (
    <form
      ...
      className={cn(isModal ? 'space-y-8' : 'space-y-6')}
    >
      <FormErrorSummary errors={errors} />

      {isModal ? (
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:gap-x-16 xl:gap-x-20 lg:items-start">
      ...
      <div
        className={cn(
          'flex gap-3 pt-2',
          isModal && 'border-t border-surface-border/70 pt-6',
        )}
      >
